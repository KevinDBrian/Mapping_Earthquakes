...
// D3. create the third tile layer that will be the background of our map.
...
L.control.layers(baseMaps, overlays).addTo(map);
  ...
  // ----------------------------- Weekly Earthquake Layer + getColor/getRadius functions -----------------------------
...
// 4. Use the same style as the earthquake data.
  function styleInfo(feature) {
    return {
      opacity: 1,
      fillOpacity: 1,
      fillColor: getMajorQuakeColor(feature.properties.mag),
      color: "#000000",
      radius: getRadius(feature.properties.mag),
      stroke: true,
      weight: 0.5
...
// 5. Change the color function to use three colors for the major earthquakes based on the magnitude of the earthquake.
  function getMajorQuakeColor(magnitude) {
    if (magnitude > 5) {
      return "#ea2c2c";
    }
    if (magnitude > 4) {
...
// // initial getRadius function does not need to be changed

    //function getMajorQuakeRadius(magnitude) {
    //     if (magnitude === 0) {
    //       return 1;
    //     }
    //     return magnitude * 4;
...
// Looping through our intervals to generate a label with a colored square for each interval.
  ...
  // Creating a GeoJSON layer with the retrieved data.
  L.geoJson(data, {
  // Use tecStyle function for weight and color.
    style: tecPlateStyle
